...
function get_all_data() {
    for (let item of mydata) {
        ...
        <div class="relative max-w-3xl mx-auto bg-gradient-to-br from-[#12021f] to-[#1a0633] 
        border border-blue-700/40 rounded-3xl shadow-2xl overflow-hidden card">
        ...
        <div class="flex justify-between items-center px-8 pt-8 pb-4">  
            ...
            <span class="verified_h1 px-4 py-2 text-lg font-bold border rounded-xl 
            bg-green-900/20 shadow-[0_0_10px_#22c55e]">  
        ...
        <div class="px-8 pb-6 border-t border-blue-700/40 pt-4">  
            ...
            <div class="flex justify-center gap-4 pt-10 items-center">  
                ...
                <button class="button_delete px-5 py-2 font-orbitron border border-red-600 
                text-red-400 rounded-xl bg-red-900/20 hover:bg-red-700/40 transition 
                shadow-[0_0_10px_#f00]">Delete</button>  

                <button class="button_edit px-5 py-2 font-orbitron border border-yellow-600 
                text-yellow-400 rounded-xl bg-yellow-900/20 hover:bg-yellow-700/40 transition 
                shadow-[0_0_10px_#ff0]">Edit</button>  

                <button class="button_cancel px-5 py-2 font-orbitron border border-blue-500 
                text-blue-300 rounded-xl bg-blue-900/20 hover:bg-blue-700/40 transition 
                shadow-[0_0_10px_#09f]">Cancel Booking</button>  

                <!-- NEW PRINT BUTTON -->
                <button class="button_print px-5 py-2 font-orbitron border border-green-500 
                text-green-300 rounded-xl bg-green-900/20 hover:bg-green-700/40 transition 
                shadow-[0_0_10px_#0f0]">Print Ticket</button>
...
container_cards.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("button_cancel")) {
        ...
        let bookingId = element_id.textContent.replace("ID :", "").trim();
    ...
    if (event.target.classList.contains("button_delete")) {
        ...
        let index = mydata.findIndex(item => item.bookingDetails[0].id == bookingId);
    ...
    // PRINT TICKET
    if (event.target.classList.contains("button_print")) {
        const card = event.target.closest(".card");
        let printContent = card.innerHTML;
